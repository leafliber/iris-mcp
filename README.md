...
|-----|-------|---------|-------|------|
| 屏幕监控 | ✅ 已实现 | ⏳ 计划中 | ⏳ 计划中 | 使用 Core Graphics |
| 键盘监控 | ✅ 已实现 | ⏳ 计划中 | ⏳ 计划中 | 使用 CGEventTap |
| 鼠标监控 | ⏳ 计划中 | ⏳ 计划中 | ⏳ 计划中 | 计划使用 CGEventTap |

**macOS 实现细节：**
- **屏幕监控**：使用 `CGDisplayCreateImage` 捕获主显示器帧
- **键盘监控**：使用 `CGEventTap` (HID级别) 监听 KeyDown/KeyUp/FlagsChanged 事件
- **鼠标监控**：架构已就绪，计划使用 CGEventTap 监听鼠标事件

### 权限要求

**macOS 辅助功能权限配置：**

macOS 的键盘和屏幕监控需要授予辅助功能权限：

1. 打开 **系统设置** → **隐私与安全性** → **辅助功能**
2. 点击 **+** 按钮
3. 添加你运行程序的应用：
   - 如果从**终端**运行：添加 **终端.app** 或 **iTerm.app**
   - 如果从 **VS Code** 运行：添加 **Visual Studio Code.app**
   - 如果从其他应用运行：添加相应的应用
4. 确保添加的应用旁边的开关已启用 ✅
5. 重启你的应用或终端
...
```bash
# 运行键盘监控测试（会提示是否有权限）
python3 examples/test_keyboard.py
```
